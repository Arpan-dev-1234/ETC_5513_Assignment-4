...
gg1<-ggplot(drug_test2, aes(x =year , y = test_and_results_count, color=test_and_results)) + 
  ...
  ggtitle("Figure1 Count Of Drug Tests And Positive Results")+
  theme_bw()
gg4<-ggplotly(gg1)
...
gg2<-ggplot(drug_test4, aes(x =year, y = deaths, color=state)) + 
  ...
  geom_point()+
  labs(y="deaths")+
  ggtitle("Figure 2 Deaths In Five States Of Australia")+
  theme_bw()
gg5<-ggplotly(gg2) 
...
gg3<-ggplot(drug_test1, aes(x=year,y=positive_drug_test , color=state)) + 
  geom_line()+
  geom_point()+
  ggtitle("Figure 3 Positive Results In Five States Of Australia")+
  theme_bw()
gg6<-ggplotly(gg3)  
...
* From **2014** number of tests was increased gradually and in **2018**
reached the level of **50000 / year**.

* The positive cases fond to be minimal till **2014**. Thereafter it
gradually started rising almost in the same proportion to that of the
tests undertaken and reached about **6000 / year** in 2018.

* **Figure-3** is drawn with year wise positive cases for the states separately
and **Figure-2** is drawn with year wise deaths occurring for states
separately.

* **ACT** has minimum (only a few) drug-positive cases over the years
although it shows a little rising after **2014 till 2018**. It maintained
single digit death count all along from **2014 to 2018** and in line with it
low positive drug cases.
* **Tas** recorded another low (but higher than ACT) in drug positive
count since **2007 to 2012** and thereafter rose to as high as **2200 in 2015** and maintained almost same rate till 2018. Death cases fell from **19 in 2007** to about **1-2 in 2012**, then rose to **9 in 2014** but fell sharply to zero in **2015** and then rose to **10 in 2016** and fell to single
digit in **2018**.
* **SA** positive cases rose almost diagonally till **201 to 5000+** and then it
decreased to about **4500 in 2016** and then again rose to 5000+ in 2018. SA has shown the death count as between **10 to 21** through out with a **rise-fall-rise-fall** pattern all along.
* **NSW** maintained almost similar pattern as lie **TAS from 2010 to 2012**
and then started rising and reached the peak with **9500 cases in 2015**. Positive cases got reduced for 2016 but rose to the high value of **9500** in next year i.e., 2017. The state recorded maximum death count. From **55 in 2010 to 40 in 2011 to 54 in 2013** to suddenly rose to above **80 in 2016, but by 2018 the figure is reduced to 70**.

* **QLD** showing very exceptional result. From almost zero in 2009, it
rose gradually to 2014 and then started rising very sharply to just
below **30000 in 2018**. The lowest (almost zero all along) death cases
recorded by the state is a great achievement by the state despite
having the highest drug-positive cases.
